...
#[test]
fn test_floating_point_1() {

    let env = String::from("(define (x (* 1.11 2))) (define (y 6.875))");
    let expr = String::from("(+ x y)");
    let val = interpret_with_environment_string(expr, env);
    assert_eq!(val, 9.095);
...
#[test]
fn test_floating_point_2() {

    let env = String::from("(define (x 17.38)) (define (y 6.003))");
    let expr = String::from("(+ 2.7 (* x y))");
    let val = interpret_with_environment_string(expr, env);
    assert_eq!(val, 107.03214);
